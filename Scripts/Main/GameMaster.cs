...
public partial class GameMaster : Node {
	...
	private void SetupAudioBusIndexes() {
		// TEMPORARY POSSIBLE SOLUTION: Sets the audio bus count to 6 by adding an index for each audio bus currently being used
		AudioServer.AddBus(1);
		AudioServer.AddBus(2);
		AudioServer.AddBus(3);
		AudioServer.AddBus(4);
		AudioServer.AddBus(5);
		
		//Assign Bus Indexes
		master_index = AudioServer.GetBusIndex("Master");
		music_index = 1; //AudioServer.GetBusIndex("Music");
		sfx_index = 2; //AudioServer.GetBusIndex("SFX");
		voice_index = 3; //AudioServer.GetBusIndex("Voice");
		male_index = 4; //AudioServer.GetBusIndex("Male");
		female_index = 5; //AudioServer.GetBusIndex("Female");
		
		// Prints the indexes of the Audio Buses (just for demonstration, can remove after)
		GD.Print("Audio Bus Indexes: \n" +
				master_index + "\n" + 
				music_index + "\n" +
				sfx_index + "\n" +
				voice_index + "\n" +
